...
SELECT
	...
	total_cases, 
	population, 
	(total_cases/population) * 100 AS Covid_Cases_Percent
FROM PortfolioProject..CovidDeaths
...
SELECT
	location, 
	population, 
	MAX(CAST(total_deaths AS INT)) AS Total_Death_Count, MAX((total_deaths/population)) * 100 AS Death_Rate
FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
GROUP BY location, population
...
-- TAKING THESE OUT AS THEY ARE NOT INCLUDED IN THE QUERIES AND WANT TO STAY CONSISTENT
...
WITH PopvsVac(continent, location, date, population, new_vaccinations, Num_getting_vaccinated) AS
		(SELECT
			CD.continent, 
			CD.location, 
			CD.date,
